...
from questions import REVIEW_TYPE_TO_QUESTIONS
...
class QuestionCreator():
    ...
    def create(self):
        subject_id = self.hive_api.get_subject_id_in_program("M", self.daily_submission.course)
        if not subject_id:
            # TODO: Method not implemented
            subject_id = self.hive_api.create_subject()
        module_id = self.hive_api.get_module_id(subject_id, "מישוב יומי")
        if not module_id:
            response = self.hive_api.create_module(subject_id, "מישוב יומי", "01", False)
        ...
        queue_id = self.hive_api.get_queue_id("מישוב יומי",module_id)
        if not queue_id:
